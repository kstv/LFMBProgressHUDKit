...
@implementation LFMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    LFMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    LFMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (LFMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (LFMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
#pragma mark --editby wangzhiwei01 字体颜色修改
    _contentColor = isLegacy ? [UIColor whiteColor] : [UIColor colorWithWhite:1.f alpha:0.9f];
...
- (void)animateIn:(BOOL)animatingIn withType:(LFMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
...
- (void)done {
    ...
    LFMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<LFMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    LFMBBackgroundView *backgroundView = [[LFMBBackgroundView alloc] initWithFrame:self.bounds];
    ...
    LFMBBackgroundView *bezelView = [LFMBBackgroundView new];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[LFMBRoundProgressView class]];

    LFMBProgressHUDMode mode = self.mode;
    if (mode == MBProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[LFMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[LFMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MBProgressHUDModeAnnularDeterminate) {
            [(LFMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[LFMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[LFMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
#pragma mark --修改菊花的背景色
            ((UIActivityIndicatorView *)indicator).color = [UIColor whiteColor];
        }
    } else if ([indicator isKindOfClass:[LFMBRoundProgressView class]]) {
        LFMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LFMBRoundProgressView appearanceWhenContainedIn:[LFMBProgressHUD class], nil];
#else
        appearance = [LFMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[LFMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((LFMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((LFMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[LFMBBarProgressView class]]) {
        LFMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [LFMBBarProgressView appearanceWhenContainedIn:[LFMBProgressHUD class], nil];
#else
        appearance = [LFMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[LFMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((LFMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((LFMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    LFMBBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(LFMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
@implementation LFMBRoundProgressView
...
@implementation LFMBBarProgressView
...
@implementation LFMBBackgroundView
...
#pragma mark -- edit by wangzhiwei01 修改菊花背景色值
                //修改菊花背景色值，
                _color = [UIColor colorWithRed:51.0/255.0f green:51.0/255.0f blue:51.0/255.0f alpha:1.0f];
            } else {
...
- (void)setStyle:(LFMBProgressHUDBackgroundStyle)style {
...
- (void)updateForBackgroundStyle {
    LFMBProgressHUDBackgroundStyle style = self.style;
    if (style == MBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
#pragma mark -- 修改菊花背景色透明度
...
@implementation LFMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [LFMBProgressHUD allHUDsForView:view];
    for (LFMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable LFMBProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    LFMBBackgroundView *backgroundView = self.backgroundView;
